const booksLibrary = [
    {
        name: "Harry Potter und der Stein der Weisen",
        author: "J.K.Rowling",
        likes: 1250,
        liked: true,
        price: 19.99,
        publishedYear: 1998,
        genre: "Fantasy",
        path : "./assets/img/Harry_Potter.png",
        comments: [
        {
            name: "Leser123",
            comment:
            "Ein faszinierendes Abenteuerbuch, das mich von der ersten Seite an gefesselt hat.",
        },
        {
            name: "Bookworm84",
            comment:
            "Eine romantische Geschichte, die mein Herz berührt und mich zum Nachdenken gebracht hat.",
        },
        {
            name: "FantasyFanatic",
            comment:
            "Eine spannende Fantasiewelt, die ich nur schwer aus der Hand legen konnte.",
        },
        {
            name: "SciFiGuru",
            comment:
            "Ein cleverer Science-Fiction-Roman mit interessanten Zeitreise-Konzepten und Charakteren.",
        },
        {
            name: "NovelLover",
            comment:
            "Ein Buch, das voller magischer Überraschungen steckt und mich begeistert hat.",
        },
        ],
    },
    {
        name: "Der Hobbit",
        author: "J.R.R. Tolkien",
        likes: 980,
        liked: false,
        price: 20.00,
        publishedYear: 2012,
        genre: "Fantasy",
        path : "./assets/img/Der_Hobbit.png",
        comments: [],
    },
    {
        name: "1984",
        author: "George Orwell",
        likes: 1520,
        liked: true,
        price: 11.95,
        publishedYear: 1991,
        genre: "dystopische Roman",
        path : "./assets/img/1984.png",
        comments: [
        {
            name: "LeserPeter",
            comment:
            "Die Handlung war fesselnd und die Charaktere unglaublich lebendig dargestellt.",
        },
        {
            name: "BookLover21",
            comment:
            "Ein romantisches Meisterwerk, das mich tief berührt und bewegt hat.",
        },
        {
            name: "FantasyNerd",
            comment:
            "Fantastische Welten und epische Abenteuer - genau mein Geschmack!",
        },
        {
            name: "SciFiEnthusiast",
            comment:
            "Die Zeitreise-Elemente waren genial und haben die Story spannend gemacht.",
        },
        {
            name: "ReadingAddict",
            comment:
            "Ein unvergessliches Buch, das mich auf eine magische Reise mitgenommen hat.",
        },
        ],
    },
    {
        name: "Das Cafe am Rande der Welt",
        author: " John Strelecky",
        likes: 750,
        liked: false,
        price: 16.0,
        publishedYear: 2007,
        genre: "Humor und anrührend",
        path : "./assets/img/Das_Cafe_am_Rande_der_Welt.png",
        comments: [
        {
            name: "BuchKenner",
            comment:
            "Ein spannendes Abenteuer, das mich von Anfang an mitgerissen hat.",
        },
        {
            name: "LeseWurm",
            comment:
            "Die Liebesgeschichte war herzergreifend und wunderschön geschrieben.",
        },
        ],
    },
    {
        name: "Percy Jackson Diebe im Olymp",
        author: "Rick Riordan",
        likes: 1300,
        liked: true,
        price: 17.75,
        publishedYear: 2006,
        genre: "Fantasy, Abenteuer, Mythologie",
        path : "./assets/img/Percy_Jackson.png",
        comments: [],
    },
    {
        name: "Percy Jackson Im Bann des Zyklopen",
        author: "Rick Riordan",
        likes: 1450,
        liked: true,
        price: 17.0,
        publishedYear: 2006,
        genre: "Fantasy",
        path : "./assets/img/Percy_Jackson2.png",
        comments: [
        {
            name: "Leser123",
            comment:
            "Ein fesselndes Abenteuer, das mich von Anfang bis Ende mitgerissen hat.",
        },
        ],
    },
];